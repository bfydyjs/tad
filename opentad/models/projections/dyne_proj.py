...
class DynELayer(nn.Module):
    def __init__(
        ...
        # --- INNOVATION 1 START: Attention for Global Context ---
        # A small network to generate attention weights for global context aggregation
        ...
        # --- INNOVATION 2 START: Dynamic Gates for Multi-scale Fusion ---
        # A convolution to generate two gates for convw and convkw
        self.gate_gen = nn.Conv1d(n_embd, n_embd * 2, 1)
        # --- INNOVATION 2 END ---
        ...
        # --- INNOVATION: Add learnable combiners ---
        self.phi_combiner = nn.Parameter(torch.zeros(2))
        self.fusion_combiner = nn.Parameter(torch.zeros(2))
        # --- END INNOVATION ---
    ...
    def reset_params(self, init_conv_vars=0):
        ...
        # --- INNOVATION: Biased initialization for regularization ---
        # Initialize to heavily favor the original, stable modules.
        # This acts as a form of curriculum learning / regularization.
        self.phi_combiner.data.copy_(torch.tensor([3.0, 0.0]))
        self.fusion_combiner.data.copy_(torch.tensor([3.0, 0.0]))
        # --- END INNOVATION ---
    ...
    def forward(self, x, mask):
        ...
        # --- INNOVATION 1: Learnable Combination of Global Context ---
        # Original global context
        phi_orig = self.global_fc(out.mean(dim=-1, keepdim=True))
        # Attention-based global context
        attn_weights = torch.softmax(self.attn_gen(out), dim=-1)
        phi_attn = self.global_fc((out * attn_weights).sum(dim=-1, keepdim=True))
        # Learnable combination
        phi_weights = torch.softmax(self.phi_combiner, dim=-1).view(-1, 1, 1)
        phi = torch.relu(phi_weights[0] * phi_orig + phi_weights[1] * phi_attn)
        # --- END INNOVATION 1 ---

        # --- INNOVATION 2: Learnable Combination of Multi-scale Fusion ---
        convw_val = self.convw(out)
        convkw_val = self.convkw(out)
        # Original fusion logic
        original_fusion = torch.relu(convw_val + convkw_val)
        # New gated fusion logic
        gates = self.gate_gen(out)
        gate1, gate2 = torch.sigmoid(gates).chunk(2, dim=1)
        gated_fusion = gate1 * convw_val + gate2 * convkw_val
        # Learnable combination
        fusion_weights = torch.softmax(self.fusion_combiner, dim=-1).view(-1, 1, 1)
        conv_out = fusion_weights[0] * original_fusion + fusion_weights[1] * gated_fusion
        # --- END INNOVATION 2 ---

        out = fc * phi + conv_out * psi + out
